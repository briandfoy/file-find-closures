# This Makefile is for the File::Find::Closures extension to perl.
#
...
#     ABSTRACT => q[functions you can use with File::Find]
#     AUTHOR => q[brian d foy <bdfoy@cpan.org>]
#     LICENSE => q[perl]
#     MAN3PODS => { lib/Closures.pm=>q[$(INST_MAN3DIR)/File::Find::Closures.3] }
#     NAME => q[File::Find::Closures]
#     PM => { lib/Closures.pm=>q[$(INST_LIBDIR)/Closures.pm] }
#     PREREQ_PM => { File::Find=>q[0] }
#     VERSION_FROM => q[lib/Closures.pm]
...
MAKE = make
FULLEXT = File/Find/Closures
BASEEXT = Closures
PARENT_NAME = File::Find
DLBASE = $(BASEEXT)
...
H_FILES  = 
MAN1PODS = 
MAN3PODS = lib/Closures.pm

# Where is the Config information that we are using/depend on
CONFIGDEP = $(PERL_ARCHLIB)$(DFSEP)Config.pm $(PERL_INC)$(DFSEP)config.h

# Where to build things
INST_LIBDIR      = $(INST_LIB)/File/Find
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/File/Find
...
TO_INST_PM = lib/Closures.pm

PM_TO_BLIB = lib/Closures.pm \
	$(INST_LIBDIR)/Closures.pm
...
DISTNAME = File-Find-Closures
DISTVNAME = File-Find-Closures-1.07
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:                File-Find-Closures' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:             1.07' >> META_new.yml
	$(NOECHO) $(ECHO) 'abstract:            functions you can use with File::Find' >> META_new.yml
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="1,07,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>functions you can use with File::Find</ABSTRACT>' >> $(DISTNAME).ppd
